...
class Isomorphism {C₀ : Sort u} {X Y : C₀} [C : Category C₀] (f : C.hom X Y) :=
  (g : C.hom Y X)
  (f_g : C.compose f g = C.id Y) -- would be nice to use ∘ instead of C.compose
  (g_f : C.compose g f = C.id X) -- same here

class SectionRetractionPair {C₀ : Sort u} {X Y : C₀} [C : Category C₀] (s : C.hom X Y) (r : C.hom Y X) :=
  (r_s : C.compose r s = C.id X) -- same here

class Monomorphism {C₀ : Sort u} {X Y Z : C₀} [C : Category C₀] (f : C.hom X Y) :=
  (mono : ∀ {g₁ g₂ : C.hom Z X}, C.compose f g₁ = C.compose f g₂ → g₁ = g₂) -- same here

class Epimorphism {C₀ : Sort u} {X Y Z : C₀} [C : Category C₀] (f : C.hom X Y) :=
